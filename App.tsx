...
import { MemoryDetailModal } from './components/MemoryDetailModal';
import { AuthScreen } from './components/AuthScreen'; // Import AuthScreen
import { Memory, TabView, Language, Theme, ChildProfile, GrowthData, SettingsView } from './types'; // SettingsView ကို ထည့်သွင်းရန် လိုအပ်နိုင်ပါသည်။
import { getTranslation } from './translations';
import { initDB, DataService, syncData, generateId } from './db';
import { supabase } from './supabaseClient'; // Import supabase
...
function App() {
  ...
  // --- NEW LOGIC: Active Profile Selection and Sorting ---

  // Child Profile ရွေးချယ်မှု function
  const handleChildProfileSelect = (profileId: string) => {
    // 1. active ID ကို update လုပ်ခြင်း
    setActiveChildId(profileId);
    localStorage.setItem('activeChildId', profileId);

    // 2. Profiles ကို ပြန်စီစဉ်ခြင်း: Active ဖြစ်သော Profile ကို ရှေ့ဆုံးသို့ ရွှေ့ခြင်း
    setChildProfiles(currentProfiles => {
        const profiles = [...currentProfiles];
        
        // Find the index of the newly active profile
        const activeIndex = profiles.findIndex(p => p.id === profileId);

        if (activeIndex > -1) {
            // Remove the active profile from its current position
    ...
    // Profile ပြောင်းပြီးနောက် Data များကို refresh လုပ်ရန်လိုအပ်နိုင်သည်။ (ဥပမာ- refreshAppData)
    // refreshAppData(); 
  };

  // Data Load လုပ်ပြီးနောက် Child Profiles ကို စီစဉ်ခြင်း
  useEffect(() => {
    // Data Loading Logic (Assumed)
    const loadChildProfiles = async () => {
        ...
        try {
            ...
            // 1. နောက်ဆုံး active ဖြစ်ခဲ့သော ID ကို localStorage မှ ရယူ
            const storedActiveId = localStorage.getItem('activeChildId');
            let initialActiveId = null;

            // 2. ရယူလာသော profiles များထဲတွင် storedActiveId ရှိ/မရှိ စစ်ဆေး
            const profileExists = profiles.some(p => p.id === storedActiveId);
            
            if (profileExists) {
                initialActiveId = storedActiveId as string;
            } else {
                // မရှိရင် (သို့မဟုတ်) storedActiveId မရှိရင်၊ ပထမဆုံး profile ကို active လုပ်
                initialActiveId = profiles[0].id;
            }

            // 3. active profile ကို ရှေ့ဆုံးသို့ ရွှေ့ခြင်း
            const sortedProfiles = [...profiles].sort((a, b) => {
                if (initialActiveId) {
                    if (a.id === initialActiveId) return -1; // active profile ကို ရှေ့ဆုံးသို့
                    if (b.id === initialActiveId) return 1;
                }
                // အခြား profiles များကိုတော့ မူရင်းအတိုင်းထား
                return 0;
            });

            setChildProfiles(sortedProfiles);
            setActiveChildId(initialActiveId);
            
            // 4. active ID ကို localStorage တွင် သိမ်းခြင်း
            if (initialActiveId) {
                localStorage.setItem('activeChildId', initialActiveId);
            }
        } catch (error) {
  ...
  }, [session, isGuest]); // session or guest mode ပြောင်းလဲရင် data ကို load လုပ်ပါ

  // --- END OF NEW LOGIC ---

  // Helper useMemo to get the active profile object
  const activeChildProfile = useMemo(() => 
    childProfiles.find(p => p.id === activeChildId), 
    [childProfiles, activeChildId]
  );

  // Other utility functions (Assumed)
  const T = useMemo(() => getTranslation(language), [language]);
  // ... (Other functions like handleThemeChange, handleLanguageChange, refreshAppData etc.)

  const tabs = [
    { id: TabView.HOME, icon: Home, label: T('Home') },
    { id: TabView.GROWTH, icon: Activity, label: T('Growth') },
    { id: TabView.MEMORY_INPUT, icon: PlusCircle, label: T('Add') },
    { id: TabView.GALLERY, icon: ImageIcon, label: T('Gallery') },
    { id: TabView.SETTINGS, icon: Settings, label: T('Settings') },
  ];

  // If not authenticated and not in guest mode, show AuthScreen
  if (!session && !isGuest && !authChecking) {
    return <AuthScreen setSession={setSession} setIsGuest={setIsGuest} T={T} />;
  }
  
  // Application Main UI
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'dark' : theme === 'light' ? 'light' : ''} bg-slate-50 dark:bg-slate-900 transition-colors duration-300 pb-24`}>
      <div className="max-w-xl mx-auto p-4 relative">
        {/* Child Profile Selector Display */}
        {childProfiles.length > 0 && activeChildProfile && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 w-fit bg-white/90 dark:bg-slate-800/90 backdrop-blur-md rounded-full shadow-lg p-2 flex items-center gap-2 z-10">
                <Baby className="w-5 h-5 text-primary" />
                <span className="text-sm font-semibold text-slate-800 dark:text-white">{activeChildProfile.name}</span>
                <button 
                    // Settings Tab ကိုဖွင့်ပြီး Profile View ကိုပြောင်းမည်။
                    onClick={() => { setActiveTab(TabView.SETTINGS); setSettingsView('PROFILES'); }}
        ...
        {/* Main Content View (Conditional Rendering) */}
        <div className="pt-20"> {/* Adjust padding based on profile selector position */}
            {activeTab === TabView.HOME && (
                <div className="space-y-6">
                    <h1 className="text-3xl font-bold text-slate-800 dark:text-white">
                        {T('Welcome back,')} {activeChildProfile?.name || ''}!
                    </h1>
                    {/* ... Home View Content (e.g., latest memories, suggestions) ... */}
                </div>
            )}
            {activeTab === TabView.GROWTH && (
                <GrowthChart T={T} growthData={growthData} activeChildId={activeChildId} />
            )}
            {activeTab === TabView.MEMORY_INPUT && (
                <StoryGenerator T={T} activeChildId={activeChildId} />
            )}
            {activeTab === TabView.GALLERY && (
                <GalleryGrid T={T} memories={memories} />
            )}
            {activeTab === TabView.SETTINGS && (
                <div className="space-y-6">
                    <h1 className="text-3xl font-bold text-slate-800 dark:text-white">{T('Settings')}</h1>
                    ...
                    {settingsView === 'MAIN' && (
                        <div className="space-y-3">
                            ...
                            <button 
                                ...
                                className="w-full flex justify-between items-center p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                            >
                                <div className="flex items-center gap-3">
                                    <Baby className="w-5 h-5 text-primary" />
                                    <span className="text-slate-800 dark:text-white">{T('Child Profiles')}</span>
                                </div>
                                <ChevronRight className="w-4 h-4 text-slate-400" />
                            </button>
                            {/* ... More Settings (Theme, Language, etc.) ... */}
                        </div>
                    )}

                    {/* Child Profile Management View */}
                    {settingsView === 'PROFILES' && (
                        <div>
                            <button onClick={() => setSettingsView('MAIN')} className="flex items-center gap-2 text-primary mb-4">
                                <ArrowLeft className="w-4 h-4" />
                                {T('Back to Settings')}
                            </button>
                            <h2 className="text-2xl font-semibold mb-4">{T('Select Active Child')}</h2>
                            <div className="space-y-3">
                                {childProfiles.map((profile) => {
                                    ...
                                    return (
                                        <div key={profile.id} className="relative">
                                            <button 
                                                onClick={() => handleChildProfileSelect(profile.id)}
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <User className={`w-5 h-5 ${isActive ? 'text-white' : 'text-primary'}`} />
                                ...
                                <button className="w-full flex items-center justify-center gap-2 p-4 border-2 border-dashed border-slate-300 dark:border-slate-700 text-slate-500 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
        ...
        {tabs.map((tab) => {
           ...
           return (
             <button 
                 ...
                 className={`relative flex items-center justify-center gap-2 h-12 rounded-full transition-all duration-500 ease-spring overflow-hidden 
                    ${isActive 
             ...
             >
                 ...
                 {isActive && <span className="text-sm font-medium whitespace-nowrap">{tab.label}</span>}
             </button>
           );
        })}
      </nav>
